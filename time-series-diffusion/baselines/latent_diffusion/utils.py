...
class diff_LDM(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.channels = config["model"]["proj_channels"]

        self.diffusion_embedding = DiffusionEmbedding(
            num_steps=config["diffusion"]["num_steps"],
            embedding_dim=config["model"]["time_embedding_dim"],
        )
    ...
    def forward(self, x, diffusion_step):
        ...
        # print("projecting through the input projection layer : ", x.shape)
        x = F.relu(x)
        x = x.reshape(B, self.channels, K, L)
        # print("after reshaping : ", x.shape)

        diffusion_emb = self.diffusion_embedding(diffusion_step)
        # print("time step embedding shape : ", diffusion_emb.shape)
        # print(x.shape, cond_info.shape, diffusion_emb.shape)

        skip = []
        # print("entering the transformer layers")
