    ...
    for epoch_no in range(model_config.n_epochs):
        ...
        with tqdm(train_loader, mininterval=5.0, maxinterval=50.0) as it:  
            for batch_no, train_batch in enumerate(it, start=1):
                ...
                reconstruction_loss = torch.nn.functional.l1_loss(pred_data, train_batch, reduction='sum')
        ...
        with torch.no_grad():
            for val_batch in val_loader:
                ...
                reconstruction_loss = torch.nn.functional.l1_loss(pred_data, val_batch, reduction='sum')
                kl_divergence_loss = torch.mean(-0.5 * torch.sum(1 + log_var - mean ** 2 - log_var.exp(), dim = 1), dim = 0)
                loss = reconstruction_loss + kl_divergence_loss 
...
if __name__ == "__main__":
    ...
    assert model_config.horizon == dataset_config.horizon
